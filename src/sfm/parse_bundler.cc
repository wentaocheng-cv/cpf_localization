...
//par
bool parse_bundler::parse_camera_data( const char *bundle_out_filename_)
{
...
bool parse_bundler::parse_data_nokey( const char *bundle_out_filename_)
{
    ////
    ...
    //std::cout << "1st step clear" << std::endl;
    ...
    for ( uint32_t i = 0; i < mNbPoints; ++i )
    {
        ...
        // std::cout << "2st step clear" << std::endl;
        ...
        // std::cout << "3st step clear" << std::endl;
        ...
        //std::cout << "4st step clear" << std::endl;
        ...
        for ( uint32_t j = 0; j < view_list_length; ++j )
        {
            ...
            // std::cout << mFeatureInfos[i].view_list[j].camera << " " << mFeatureInfos[i].view_list[j].key << " " << mFeatureInfos[i].view_list[j].x << std::endl;
        }
        //std::cout << "5st step clear" << std::endl;
    }
...
bool parse_bundler::parse_data( const char *bundle_out_filename_, const char *image_list_filename )
{
    ...
    {
        cam_feature_infos[i].clear();
        mCameras[i].point_list.clear();
        mCameras[i].point_pos.clear();
    }
    //std::cout << "1st step clear" << std::endl;
    ...
    for ( uint32_t i = 0; i < mNbPoints; ++i )
    {
        ...
        for ( uint32_t j = 0; j < view_list_length; ++j )
        {
            instream >> mFeatureInfos[i].view_list[j].camera >> mFeatureInfos[i].view_list[j].key >> mFeatureInfos[i].view_list[j].x >> mFeatureInfos[i].view_list[j].y;
            cam_feature_infos[mFeatureInfos[i].view_list[j].camera].push_back( std::make_pair( i, j ) );
            mCameras[mFeatureInfos[i].view_list[j].camera].point_list.push_back(i);
            mCameras[mFeatureInfos[i].view_list[j].camera].point_pos.push_back(mFeatureInfos[i].view_list[j].x);
            mCameras[mFeatureInfos[i].view_list[j].camera].point_pos.push_back(mFeatureInfos[i].view_list[j].y);
            // std::cout << mFeatureInfos[i].view_list[j].camera << " " << mFeatureInfos[i].view_list[j].key << " " << mFeatureInfos[i].view_list[j].x << std::endl;
        }
        //std::cout << "5st step clear" << std::endl;
    }
    ...
    keyfilenames.resize( mNbCameras );
    {
        ...
        for ( uint32_t i = 0; i < mNbCameras; ++i )
        {
            ...
            keyfilenames[i] = tmp;
            std::cout << keyfilenames[i] << std::endl;
        }
    ...
    for ( uint32_t i = 0; i < mNbCameras; ++i )
    {
        ...
        // std::string jpg_filename( keyfilenames[i] );
        ...
        // mCameras[i].width = img_width;
        // mCameras[i].height = img_height;
        ...
        for ( uint32_t j = 0; j < nb_des; ++j )
        {
            ...
            // if(i==3196)
            //{
            //std::cout << feature_id_keyfile << " "<<  (uint32_t) keypoints.size() << std::endl;
            //}
            ...
            else
            {
                ...
                //wentao cheng
                mFeatureInfos[feature_id].view_list[view_list_id].x = keypoints[feature_id_keyfile].x;
                mFeatureInfos[feature_id].view_list[view_list_id].y = keypoints[feature_id_keyfile].y;
                // set scale and orientation of the keypoint in the view list
...
bool parse_bundler::load_from_binary( const char *filename, const int format )
{
    ...
    for ( uint32_t i = 0; i < mNbPoints; ++i )
    {
        ...
        for ( uint32_t j = 0; j < size_view_list; ++j )
        {
            ...
            //added by cwt
            //this step is only for the info data which contains cameras information
            //for dubro and rome, it should be commented
           // mCameras[cam_id].point_list.push_back(i);
...
bool parse_bundler::load_from_binary_nokey( const char *filename, const int format )
{
    ////
    // initialize the data structure for the 3D points
    for ( uint32_t i = 0; i < mNbPoints; ++i )
...
bool parse_bundler::parse_data_from_binary( const char *bundle_out_filename_,
        std::vector< feature_3D_info > &feature_infos, const char *id_file )
{
    ...
    //read id file
    std::ifstream id_instream( id_file, std::ios::in );
    uint32_t  id_index[mNbPoints];
    for ( uint32_t i = 0; i < mNbPoints; ++i )
    {
        id_instream >> id_index[i];
        mFeatureInfos[i] = feature_3D_info(feature_infos[id_index[i]]);
    }
    id_instream.close();
}