...
int main (int argc, char **argv)
{
	...
	std::vector< std::pair< double, uint32_t > > ori_voting_corrs_score;
	...
	for ( uint32_t i = 0; i < nb_keyfiles; ++i, nb_query += 1.0)
	{
		...
		for (int j = 0; j < corrs.size(); j++)
		{
			...
			double ori_score = exp(-1.0f * oper * oper);
			...
			ori_voting_corrs_score.push_back(std::make_pair(ori_score, j));
		}
		//std::cout << "!" << std::endl;
		...
		for (int j = 0; j < camera_infos.size(); j++)
		{
			if (camera_infos[j].vote_list.size() > 0)
			{
				for (int k = 0; k < camera_infos[j].vote_list.size(); k++)
				{
					...
					camera_infos[j].probability += ori_voting_corrs_score[camera_infos[j].vote_list[k]].first;
