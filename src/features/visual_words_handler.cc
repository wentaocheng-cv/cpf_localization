...
std::vector<float> visual_words_handler::get_trained_centers()
{
  int center_size = mTrainedCenters.rows;
  std::vector< float > cluster_centers_( center_size * uint32_t(128), 0.0f );
  uint32_t index = 0;
  for ( uint32_t i = 0; i < center_size; ++i )
  {
    for ( uint32_t j = 0; j < 128; ++j )
      cluster_centers_[index + j] = (mTrainedCenters.data)[ index + j ];
    index += (uint32_t) 128;
  }
...
//wentao cheng
bool visual_words_handler::fast_cluster( std::vector< float > database_sift, uint32_t nb_db_sift )
{
  mFlannFeatures.free();
  mFlannFeatures.data = new float[128 * nb_db_sift];
  mFlannFeatures.rows = nb_db_sift;
  mFlannFeatures.cols = 128;
  for ( uint64_t i = 0; i < nb_db_sift; ++i )
  {
    for ( uint64_t j = 0; j < 128; ++j )
    {
      uint64_t index = i * 128 + j;
      mFlannFeatures[i][j] = (float) database_sift[index];
     // std::cout << (float) database_sift[index] << " ";
    }
  }

  std::cout << "number of descriptors " << mFlannFeatures.rows << std::endl;
  mTrainedCenters.free();
  mTrainedCenters.data = new float[100000 * 128 ];
  mTrainedCenters.rows = (size_t) 100000;
  mTrainedCenters.cols = 128;
  ...
  for ( size_t i = 0; i < 100000; ++i )
  {
    for ( size_t j = 0; j < 128; ++j )
    {
      mTrainedCenters[i][j] = 0;
    }
  }

  int true_clusters = flann::hierarchicalClustering< flann::L2< float > >( mFlannFeatures,mTrainedCenters,flann::KMeansIndexParams(64, 100, FLANN_CENTERS_RANDOM) );
  std::cout << "generate " << true_clusters << " centers" << std::endl;
  // for(int i = 0; i < 10; i++)
  // {
  //   for(int j = 0 ; j < 128; j++)
  //   {
  //     std::cout << mClusterCentersFlann[i][j] << " ";
  //   }
  //   std::cout << std::endl;
  // }
}
