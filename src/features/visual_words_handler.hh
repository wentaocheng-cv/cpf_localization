...
class visual_words_handler
{
  public:
    ...
    **/
    ...
    void get_cluster_centers_flann( std::vector< float > &centers_ );
     ...
     //! get the cluster centers
    std::vector<float> get_trained_centers();
    ...
    **/
    ...
    //wentao 
    bool fast_cluster( std::vector< float > database_sift, uint32_t nb_db_sift );
  ...
  private:
    ...
    //! the matrix containing the trained cluster centers
    flann::Matrix< float > mTrainedCenters;
