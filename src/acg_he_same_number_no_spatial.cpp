...
int main (int argc, char **argv)
{
	...
	std::ifstream in_pick_numb( argv[17], std::ios::in );

	std::vector<int> pick_supp;
	std::vector<int> pick_comple;
	pick_supp.resize(800);
	pick_comple.resize(800);

	while (!in_pick_numb.eof())
	{
		int tmp_id; int tmp_supp; int tmp_comple;
		in_pick_numb >> tmp_id >> tmp_supp >> tmp_comple;
		pick_supp[tmp_id] = tmp_supp;
		pick_comple[tmp_id] = tmp_comple;
	}
	////
	...
	for ( uint32_t i = 0; i < nb_keyfiles; ++i, nb_query += 1.0)
	{
		...
		int nb_picked_supp = 0;
		for (int j = 0; j < comple_corrs_in_top_img.size(); j++ )
		{
			int cur_id = corrs_in_top_img[j].second;
			if(nb_picked_supp >= pick_comple[i])
				break;
			// std::cout << "bin " << w_idx * w_cell + h_idx << std::endl;
		...
		std::cout << "done pick the global best corrs " << nb_picked_supp << std::endl;
...
/*record the number of support and complementary matches*/
...
int main (int argc, char **argv)
{
	...
	for ( uint32_t i = 0; i < nb_keyfiles; ++i, nb_query += 1.0)
	{
		...
		std::cout << "done pick the global best corrs " << chosen_pt.size() << std::endl;
		int support_match_nb = chosen_pt.size();
		...
		std::cout << "after spatial augmention " << chosen_pt.size() << std::endl;
		int comple_match_nb = chosen_pt.size() - support_match_nb;
		...
		//for writing the voted images information
		ofs_details  << i << " " << support_match_nb << " " << comple_match_nb << std::endl;
		int cur_write_img = 0;
		for (int j = 0; j < camera_rank.size(); j++)
		{
			cur_write_img++;
