...
int main (int argc, char **argv)
{
	...
	std::vector < std::bitset<128> > all_binary_descriptors;
	...
	for (int i = 0; i < nb_descriptors; ++i) {
		std::string tmp_desc;
		ifs >> tmp_desc;
		all_binary_descriptors[i] = std::bitset<128>(tmp_desc);
	}
	...
	for ( uint32_t i = 0; i < nb_keyfiles; ++i, nb_query += 1.0)
	{
		...
		for ( size_t j = 0; j < nb_loaded_keypoints; ++j )
		{
			...
			Eigen::Matrix<float, 128, 1> proj_sift = projection_matrix * query_sift.col(j);
		...
		for (int j = 0; j < corrs.size(); j++)
		{
			...
			//scheme 3
			double oper;
			if (desc_dist[j].second <= 16)
			{
				oper = 16.0f / 32.0f;
			}
			else {
				oper =  (double)desc_dist[j].second / 32.0f;
			}
			double score = ( hamming_ratio * exp(-1.0f * oper * oper)) / (oper * oper)  ;
		...
		{
			pick_vfci_img++;
			if (pick_vfci_img > top_rank_k)
				break;
			int top_cam_pick = camera_rank[j].second;
			for (int k = 0; k < camera_infos[top_cam_pick].vote_list.size(); k++)
			{
				int cur_bin = assoc_bin[camera_infos[top_cam_pick].vote_list[k]];
				if (bin[cur_bin].contained >= bin[cur_bin].quota)
					break;
				if (!occupied[camera_infos[top_cam_pick].vote_list[k]])
				{
					if (new_corrs_score[camera_infos[top_cam_pick].vote_list[k]].first >= score_threshold)
					{
						chosen_pt.push_back(camera_infos[top_cam_pick].vote_list[k]);
						occupied[camera_infos[top_cam_pick].vote_list[k]] = true;
						bin[cur_bin].contained++;
						chosen_flag[camera_infos[top_cam_pick].vote_list[k]] = 2;
...
// 					// if (new_corrs_score[tmp_camera_infos[j].vote_list[k]].first >= score_threshold)
// 					// {
// 					chosen_pt.push_back(camera_infos[top_cam].vote_list[k]);
// 					picked[camera_infos[top_cam].vote_list[k]] = true;
// 					//}
// 				}
// 			}
