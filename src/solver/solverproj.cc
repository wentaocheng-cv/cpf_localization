...
void SolverProj::clear( void )
{
    ...
    corresp_position.clear();
    corresp_2d_position.clear();
    corresp_index.clear();
}
...
//wentao cheng
std::vector <float >& SolverProj::getHypotheisPosition()
{
    if (mv_corresp.size() ==  6)
    {
        for (int i = 0; i < 6; i++)
        {
            corresp_position.push_back(float(mv_corresp[i].m_point3D[0]));
            corresp_position.push_back(float(mv_corresp[i].m_point3D[1]));
            corresp_position.push_back(float(mv_corresp[i].m_point3D[2]));
        }
    }
    return corresp_position;
}

//wentao cheng
std::vector <float >& SolverProj::getHypotheis2DPosition()
{
    if (mv_corresp.size() ==  6)
    {
        for (int i = 0; i < 6; i++)
        {
            corresp_2d_position.push_back(float(mv_corresp[i].m_point2D[0]));
            corresp_2d_position.push_back(float(mv_corresp[i].m_point2D[1]));
            
        }
    }
    return corresp_2d_position;
}

void SolverProj::addIndex(const int index)
{
    corresp_index.push_back(index);
}
...
bool SolverProj::scaleCorrespondences( void )
{
    ...
    //Vector3D close_to(0.0,0.0,100.0);
    // Vector3D close_to(-63.0, -17.0, -166.0);
    // for ( cIter = mv_corresp.begin();
    //         cIter != cEnd;
    //         ++cIter )
    // {
    //     Vector3D move = (close_to - cIter->m_point3D) * 0.75;
    //     cIter->m_point3D += move;
    // }
    ...
    //
    //Vector3D close_to(0.0,0.0,100.0);
    //Vector3D pesudo_move =  center3D - close_to;
            ...
            ++cIter )
    {
        ...
        //pt2 = cIter->m_point3D - pesudo_move;
    ...
    //manmade scale 2
    //scale2 = 0.005;
   // std::cout << " scale 2 " << scale2 << " and the six pos are " << std::endl;
            ...
            ++cIter )
    {
        ...
        //std::cout << cIter->m_point3D << std::endl;
    }
    ...
    //std::cout << "centroid " << center3D[ 0 ] << " " << center3D[ 1 ] << " " << center3D[ 2 ] << std::endl;
...
void SolverProj::multForP3P( ProjMatrix & _mat )
{
    //  _mat.multLeft( m_matScaleImgInv );
    // _mat.multRight( m_matScaleWorld );
...
bool SolverProj::rescaleCorrespondences( const Vector3D & estimate_center, const double move_scale )
{
    CorrespondenceVec::iterator cIter, cEnd;
    cEnd = mv_corresp.end();
    ...
    for ( cIter = mv_corresp.begin();
            ...
            ++cIter )
    {
        Vector3D move = (estimate_center - cIter->m_point3D) * move_scale;
        cIter->m_point3D += move;
    }
...
bool SolverProj::computeLinearNewWithRescale( const Vector3D & estimate_center, const double move_scale )
{
    int nrows, ncols;
    ...
    if ( !rescaleCorrespondences(estimate_center, move_scale) )
        return false;
        ...
        const Vector3D & _point3D )
{
    ...
    Vector3D tmp = _point3D;
    //Vector3D close_to(-63.0, -17.0, -166.0);

    //Vector3D move = (close_to - tmp) * 0.50;
   // tmp += move;

    //m_projectionMatrix.project( _point3D, imgPoint );
    m_projectionMatrix.project( tmp, imgPoint );
