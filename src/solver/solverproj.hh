...
public:
    ...
    bool computeLinearNewWithRescale( const Vector3D & estimate_center, const double move_scale );
    ...
    //wentao cheng
    int getHypotheisSize()
    {
        return mv_corresp.size();
    }

    std::vector <float >& getHypotheisPosition();
    std::vector <float >& getHypotheis2DPosition();

    std::vector <int >& getIndex( )
    {
        return corresp_index;
    }

    CorrespondenceVec get_corrs()
    {
        return mv_corresp;
    }
//! Function that scales original features, returns false if one of the scaling factors is too close to 0
    ...
    void multForP3P (ProjMatrix & _mat);
...
protected:
    ...
    bool rescaleCorrespondences( const Vector3D & estimate_center, const double move_scale  );
    ...
    //wentao cheng
    std::vector<float> corresp_position;
    std::vector<float> corresp_2d_position;
    std::vector<int> corresp_index;
