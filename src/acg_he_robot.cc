...
int main (int argc, char **argv)
{
	...
	//this is just for reading the keypoints positions in the database image.
	parse_bundler parser_1;
	std::string bundle_file_1( argv[10] );
	//parser_1.load_from_binary( bundle_file_1.c_str(), 0 );


	//for aachen and robot+car

	parser_1.load_from_binary( bundle_file_1.c_str(), 1 );


	std::vector< feature_3D_info >& feature_infos_ref = parser_1.get_feature_infos();
	// std::cout << "info 2 size " << feature_infos_ref.size() << std::endl;
	// for(int i = 0; i < feature_infos_ref.size(); i++)
	// {
	//   for(int j = 0; j < feature_infos_ref[i].view_list.size();j++)
	//   {
	//     std::cout << feature_infos_ref[i].view_list[j].x << std::endl;
	//   }
	// }


	// //get the 1389 image's key file
	// int key_in_1389 = 0;
	// for (int i = 0; i < feature_infos_ref.size(); i++)
	// {
	//   for (int j = 0; j < feature_infos_ref[i].view_list.size(); j++)
	//   {
	//     if(feature_infos_ref[i].view_list[j].camera == 1389)
	//     {
	//       std::cout << i << std::endl;
	//       ofs_2d << feature_infos_ref[i].view_list[j].y << " " << feature_infos_ref[i].view_list[j].x
	//       <<  " " <<feature_infos_ref[i].view_list[j].orientation <<
	//        " " << feature_infos_ref[i].view_list[j].orientation << std::endl;
	//        for(int k = 0; k < 128; k++)
	//        {
	//         ofs_2d << (int)feature_infos_ref[i].descriptors[j*128 +k] << " ";
	//        }
	//        ofs_2d << std::endl;
	//        key_in_1389++;
	//     }
	...
	// std::cout << "in 1389 " << key_in_1389 << std::endl;
	// return 0;
	...
	int camera_location_flag = 1000;
	std::string str1 ("left");
	std::string str2 ("rear");
	std::string str3 ("right");
	...
	for ( uint32_t i = 0; i < nb_keyfiles; ++i, nb_query += 1.0)
	{
		...
		std::size_t found = jpg_filename.find(str1);
		if (found != std::string::npos)
		{
			std::cout << "this is a left camera" << std::endl;
			camera_location_flag = 0;
		}
		found = jpg_filename.find(str2);
		if (found != std::string::npos)
		{
			std::cout << "this is a rear camera" << std::endl;
			camera_location_flag = 1;
		}
		found = jpg_filename.find(str3);
		if (found != std::string::npos)
		{
			std::cout << "this is a right camera" << std::endl;
			camera_location_flag = 2;
		}
		...
		for (int j = 0; j < corrs.size(); j++)
		{
			...
			//double hamming_ratio = cur_avg_feature_distance / (double)desc_dist[j].second;
			double hamming_ratio = cur_avg_feature_distance / (double)(desc_dist[j].second + 1);


			// std::cout << " hamming ratio " << hamming_ratio << std::endl;
			//double oper = (double)desc_dist[j].second / 16.0f;
			//double score = (hamming_ratio * exp(-1.0f * oper * oper)) / ((double)sqrt(q_set_size)  * oper * oper);

			//double score = (hamming_ratio * exp(-1.0f * oper * oper)) / (oper * oper);

			/*a new function to make 1 ht not much high*/
			//double score = hamming_ratio * pow(exp(-1.0f * oper * oper),3);
			...
			//double score = hamming_ratio * pow(exp(-1.0f * oper * oper), 4);
			double score = ( hamming_ratio * exp(-1.0f * oper * oper)) / (oper * oper)  ;
			//the old function
			// double oper =  (double)desc_dist[j].second / 16.0f;
		...
		for (int j = 0; j < corrs.size(); j++)
		{
			...
			// {
			// 	if (feature_infos[cur_3d_pt].matched_query.size() <= 3)
			// 	{
			// 		if (query_set[corrs[j].first].size() <= 50)
			// 		{
			...
			if (corrs_ratio_test[j].first <= 3.2)
			{
				int cur_3d_pt = corrs[j].second;
		...
		for (int j = 0; j < camera_infos.size(); j++)
		{
			if (camera_infos[j].vote_list.size() > 0)
			{
				...
				//camera_infos[j].probability /= log2((double)camera_infos[j].point_list.size());
				double nb_pt_per_db = camera_infos[j].point_list.size();
				//camera_infos[j].probability /= nb_pt_per_db;
				//in best version,we dont use sqrt
			...
			if (camera_infos[j].valid_corrs_nb >= 3)
		...
		//a threshold for the new pow3 score function
		...
		for (int j = 0; j < camera_rank.size(); j++)
		{
			...
			if (camera_infos[top_cam].avg_hamming_distance <= image_distance_threshold)
			{
				...
				//double update_step =  (double)confident_pt_nb * score_threshold / (double)augment_pt_nb;
		...
		{
			nb_selected_best++;
			if (nb_selected_best > nb_best_image)
				break;
			int top_cam = camera_rank[j].second;
			if (camera_infos[top_cam].avg_hamming_distance <= image_distance_threshold)
			{
		...
		int write_nb_img = 100;
		int cur_write_img = 0;
		for (int j = 0; j < camera_rank.size(); j++)
		{
			cur_write_img++;
			if (cur_write_img > write_nb_img)
				break;
			float avg_desc_dist = 0;
			int top_cam = camera_rank[j].second;
			// ofs_details << "image -------------------- " << camera_infos[top_cam].id
			//             << " " << camera_infos[top_cam].point_list.size() << " rank " << j << std::endl;
			...
			//ofs_details << camera_infos[top_cam].id << " average distance " << camera_infos[top_cam].avg_hamming_distance << std::endl;
		...
		ofs_2d << i << " " << chosen_pt.size() << " " << camera_location_flag <<
		       " " << jpg_filename << std::endl;
		...
		if (camera_location_flag == 0)
		{
			for (int j = 0; j < chosen_pt.size(); j++ )
			{
				ofs_2d << keypoints[corrs[chosen_pt[j]].first].x + half_w - 500.107605
				       << " " << 511.461426 - (half_h - keypoints[corrs[chosen_pt[j]].first].y) << std::endl;
				ofs_3d << feature_infos[corrs[chosen_pt[j]].second].point.x << " "
				       << feature_infos[corrs[chosen_pt[j]].second].point.y << " "
				       << feature_infos[corrs[chosen_pt[j]].second].point.z << std::endl;
			...
			{
				//std::cout << feature_infos[corrs[chosen_pt[j]].second].matched_query.size() << " ";
				ofs_2d << keypoints[corrs[potential_chosen_pt[j]].first].x + half_w - 500.107605 << " "
				       << 511.461426 - (half_h - keypoints[corrs[potential_chosen_pt[j]].first].y ) << std::endl;
				ofs_3d << feature_infos[corrs[potential_chosen_pt[j]].second].point.x << " "
				       << feature_infos[corrs[potential_chosen_pt[j]].second].point.y << " "
				       << feature_infos[corrs[potential_chosen_pt[j]].second].point.z << std::endl;
			}
		}
		else if (camera_location_flag == 1)
		{
			for (int j = 0; j < chosen_pt.size(); j++ )
			{
				ofs_2d << keypoints[corrs[chosen_pt[j]].first].x + half_w - 508.222931
				       << " " << 498.187378 - (half_h - keypoints[corrs[chosen_pt[j]].first].y) << std::endl;
				ofs_3d << feature_infos[corrs[chosen_pt[j]].second].point.x << " "
				       << feature_infos[corrs[chosen_pt[j]].second].point.y << " "
				       << feature_infos[corrs[chosen_pt[j]].second].point.z << std::endl;
			...
			{
				//std::cout << feature_infos[corrs[chosen_pt[j]].second].matched_query.size() << " ";
				ofs_2d << keypoints[corrs[potential_chosen_pt[j]].first].x + half_w - 508.222931 << " "
				       << 498.187378 - (half_h - keypoints[corrs[potential_chosen_pt[j]].first].y ) << std::endl;
				ofs_3d << feature_infos[corrs[potential_chosen_pt[j]].second].point.x << " "
				       << feature_infos[corrs[potential_chosen_pt[j]].second].point.y << " "
				       << feature_infos[corrs[potential_chosen_pt[j]].second].point.z << std::endl;
			}
		}
		else if (camera_location_flag == 2)
		{
			for (int j = 0; j < chosen_pt.size(); j++ )
			{
				ofs_2d << keypoints[corrs[chosen_pt[j]].first].x + half_w - 502.503754
				       << " " << 490.259033 - (half_h - keypoints[corrs[chosen_pt[j]].first].y) << std::endl;
				ofs_3d << feature_infos[corrs[chosen_pt[j]].second].point.x << " "
				       << feature_infos[corrs[chosen_pt[j]].second].point.y << " "
				       << feature_infos[corrs[chosen_pt[j]].second].point.z << std::endl;
			...
			{
				//std::cout << feature_infos[corrs[chosen_pt[j]].second].matched_query.size() << " ";
				ofs_2d << keypoints[corrs[potential_chosen_pt[j]].first].x + half_w - 502.503754 << " "
				       << 490.259033 - (half_h - keypoints[corrs[potential_chosen_pt[j]].first].y ) << std::endl;
				ofs_3d << feature_infos[corrs[potential_chosen_pt[j]].second].point.x << " "
				       << feature_infos[corrs[potential_chosen_pt[j]].second].point.y << " "
				       << feature_infos[corrs[potential_chosen_pt[j]].second].point.z << std::endl;
			}
...
/****this is the version when I achieve the first results on robot car, the result is pretty good
...
// 				for (int k = 0; k < camera_infos[j].vote_list.size(); k++)
// 				{
// 					if (corrs_score[camera_infos[j].vote_list[k]].first >= 0.6)
// 					{
// 						camera_infos[j].probability += corrs_score[camera_infos[j].vote_list[k]].first;
// 						camera_infos[j].valid_corrs_nb++;
// 						camera_infos[j].avg_hamming_distance += desc_dist[camera_infos[j].vote_list[k]].second;
